...
// Razorpay signs the exact bytes it sends. Next.js must not parse the body first:
// JSON.stringify(JSON.parse(x)) !== x whenever the payload carries floats, unicode
// escapes or different whitespace, which silently rejects every legitimate webhook.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const event = webhookEventSchema.parse(parsedBody) as unknown as RazorpayWebhookEvent;

        // Deduplication is handled durably further down by the persisted payment
        // status (`existingData?.status === "captured"`). The previous in-memory Set
        // was lost on restart, not shared across replicas, self-evicted under load,
        // and keyed on second-granularity `created_at` — so two genuine events in the
        // same second collided and the second was silently dropped.
        ...
        // Replays are absorbed inside processWebhookEvent, which short-circuits on the
        // persisted payment status rather than on process-local memory.
